...
def steiner_tree(G, terminal_nodes, weight="weight", method=None):
    """
    Return an approximation to the minimum Steiner tree of a graph using the specified method.
    """
    method = method or "mehlhorn"

    if method not in ALGORITHMS:
        raise ValueError(f"{method} is not a valid choice for an algorithm.")
    ...
    if G.is_multigraph():
        ...
        T = G.edge_subgraph(weight_min_edges)
    else:
