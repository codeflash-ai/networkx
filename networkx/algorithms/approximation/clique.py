...
def large_clique_size(G):
    ...
    nodes_by_degree = sorted(degrees, key=degrees.get, reverse=True)

    def _clique_heuristic(G, U, size, best_size):
        if not U:
            return max(best_size, size)

        # Select a pivot node u.
        u = max(U, key=degrees.get)
        U.remove(u)

        # Set of neighbors of u with degree greater than or equal best_size.
