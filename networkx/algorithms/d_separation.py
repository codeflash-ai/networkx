...
def _reachable(G, x, a, z):
    ...
    for node in x:
        if G.pred[node]:
            ...
            processed.add((True, node))
        if G.succ[node]:
            queue.append((False, node))
            processed.add((False, node))

    while queue:
        e, v = queue.popleft()
        for n in G.pred[v]:
            if (False, n) not in processed and _pass(e, v, False, n):
                queue.append((False, n))
                processed.add((False, n))
        for n in G.succ[v]:
            if (True, n) not in processed and _pass(e, v, True, n):
                queue.append((True, n))
                processed.add((True, n))
